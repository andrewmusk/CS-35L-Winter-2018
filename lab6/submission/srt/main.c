...
void *process(void *numThreads)
{
  long threadNum = (long)numThreads;

  /* for every pixel */
    for( int px=threadNum; px<width; px+=no_threads )
    {
...
{
    no_threads = argc == 2 ? atoi( argv[1] ) : 0;
  ...
  pthread_t threadArr[no_threads];    
  long currentThread;

    for (currentThread = 0; currentThread < no_threads; currentThread++) 
    {
        if ( pthread_create(&threadArr[currentThread], NULL, process, (void*)(currentThread)) )
        {
            printf("Error whening creating thread");
            exit(1);
        }
    }
    ...
    for(int i = 0; i < no_threads; i++)
    {
      ...
      {
        printf("Error whening joining thread");
        exit(1);
      }
    }
